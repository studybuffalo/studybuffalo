...
class TestCalendarCodeDelete(TestCase):
    def setUp(self):
        ...
        self.valid_args = {'code_id': self.shift_code.id}
        self.valid_url = '/rdrhc-calendar/shifts/delete/{}/'.format(
            self.shift_code.id
        )

    def test_302_response_if_not_logged_in(self):
        response = self.client.get(
            reverse('rdrhc_calendar:code_delete', kwargs=self.valid_args)
        )
    ...
    def test_code_list_template(self):
        ...
        self.assertTemplateUsed(response, 'rdrhc_calendar/shiftcode_delete.html')
