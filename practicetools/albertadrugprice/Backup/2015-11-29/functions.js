...
function chooseResult(selection) {
	...
	var query = selection.dataset.index;
...
function addLCA(result) {
	temp = {genericName: result[0].genericName,
			strength: result[0].strength,
			route: result[0].route,
			dosageForm: result[0].dosageForm,
			brandName: "LCA",
			unitPrice: result[0].unitPrice,
			unitIssue: result[0].unitIssue};
	
	for (var i = 0; i < result.length; i++) {
		if (result[i].unitPrice < temp.unitPrice) {
			temp = {genericName: result[i].genericName,
					strength: result[i].strength,
					route: result[i].route,
					dosageForm: result[i].dosageForm,
					brandName: "LCA",
					unitPrice: result[i].unitPrice,
					unitIssue: result[i].unitIssue};
		}
	}
...
function processResult(array) {
	...
	// Medication Name
	medicationSpan.innerHTML = "<strong>" + result[0].genericName + 
							   "</strong><br><em>" + result[0].strength + 
							   " " + result[0].route + " " + 
							   result[0].dosageForm + "</em>";
	...
	for (var i = 0; i < result.length; i++) {
		...
		tempOption.text = result[i].brandName;
		tempOption.dataset.cost = result[i].unitPrice;
		tempOption.dataset.unit = result[i].unitIssue;
