...
function showDiv() {
	...
	if (radio[0].checked) {
		...
		div[2].style.display = "none";
		div[3].style.display = "none";
		div[4].style.display = "initial";
		div[5].style.display = "none";
		div[6].style.display = "initial";
		
		graphDiv.style.display = "initial";
...
function generateGraph(dose, tau, ke, vd) {
	...
	xScale = xMax / (canvasWidth - canvasPadding);	
	...
	yScale =  yMax / (canvasHeight - canvasPadding);	
	...
	context.fillRect(canvasPadding, 0, canvasWidth - canvasPadding, 
					canvasHeight - canvasPadding);
	
	//Create x-axis
	context.lineWidth = 2;
	context.beginPath();
	tempX = canvasPadding - 1;
	tempY = canvasHeight - canvasPadding;
	...
	context.beginPath();
	tempX = canvasPadding;
	tempY = canvasHeight - canvasPadding - (10 / yScale);
	context.moveTo(tempX, tempY);
	tempX = canvasWidth;
	context.lineTo(tempX, tempY);
	context.stroke();
	...
	context.beginPath();
	tempX = canvasPadding;
	tempY = canvasHeight - canvasPadding - (15 / yScale);
	context.moveTo(tempX, tempY);
	tempX = canvasWidth;
	context.lineTo(tempX, tempY);
	context.stroke();
	...
	context.beginPath();
	tempX = canvasPadding;
	tempY = canvasHeight - canvasPadding - (20 / yScale);
	context.moveTo(tempX, tempY);
	tempX = canvasWidth;
	context.lineTo(tempX, tempY);
	context.stroke();
	...
	{
		xAxis = i * 4;
		tempX = canvasPadding + (xAxis / xScale);
		tempY = canvasHeight - canvasPadding + 4;
	...
	{
		yAxis = i * 5;
		tempY = canvasHeight - canvasPadding - (yAxis / yScale);
		tempX = canvasPadding - 4;
	...
	for (i = 0; i < coordinates.length; i++) {
		tempX = (coordinates[i].x / xScale) + canvasPadding;
		tempY = canvasHeight - canvasPadding - (coordinates[i].y / yScale);
		context.lineTo(tempX, tempY);
	...
	tempX = ((canvasWidth - canvasPadding) / 2) + 15;
	...
	context.save();
	tempX = 5;
	tempY = ((canvasHeight - canvasPadding) / 2);
...
function copyLevel() {
	...
	if (type[1].checked) {
		...
		vd = document.getElementById("PK-Empiric-Vd").value;
		ke = document.getElementById("PK-Empiric-Ke").value;
...
function mathEmpiricPK(vdWeight, weight, weightSource, vd, peak, trough, dose, 
	...
	//Interval
	equation = "\\begin{align}";
	equation += "\\tau & = \\frac{log(Desired\\ Peak\\ (mg/L) / " +
				"Desired\\ Trough\\ (mg/L))}{k_e\\ h^{-1}} &\\\\[5pt]";
	
	if (peak > 0 && trough > 0 && ke > 0) {
		equation += "& = \\frac{log(" + peak + "\\ mg/L / " + 
					trough + "\\ mg/L)}{" + ke + "\\ h^{-1}} &\\\\[5pt]";
